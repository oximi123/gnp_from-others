        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    ts = (torch.ones([S,1]) * 0).type(pts.dtype).to(pts.device)
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, rays_t, staged=False, bg_color=bg_color, perturb=True, **vars(self.opt))
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, rays_t, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        outputs = self.model.render(rays_o, rays_d, rays_t, staged=True, bg_color=bg_color, perturb=perturb, **vars(self.opt))
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        self.error_type = train_loader._data.error_type
    ...
    def test(self, loader, save_path=None):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                img = (preds[0].detach().cpu().numpy() * 255).astype(np.uint8)
                print(np.mean(img), img.shape)
                imageio.imsave(path, img)
                render_imgs.append(img)
                depth_img = (preds_depth[0].detach().cpu().numpy() * 255).astype(np.uint8)
                imageio.imsave(path_depth,depth_img)
                pbar.update(loader.batch_size)
                render_depth_imgs.append(depth_img)

        # save mp4
        writer = imageio.get_writer(os.path.join(save_path, f'video.mp4'), fps=10)
        for im in render_imgs:
            writer.append_data(im)
        writer.close()

        writer = imageio.get_writer(os.path.join(save_path, f'video_depth.mp4'), fps=10)
        for im in render_depth_imgs:
            writer.append_data(im)
        writer.close()
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
                    imageio.imsave(save_path,(preds[0].detach().cpu().numpy() * 255).astype(np.uint8))
                    imageio.imsave(save_path_depth, (preds_depth[0].detach().cpu().numpy() * 255).astype(np.uint8))
    ...
    def load_checkpoint(self, checkpoint=None):
        ...
        if self.lr_scheduler and 'lr_scheduler' in checkpoint_dict:
           try:
               ...
               self.log("[INFO] Milestones %s"%(str(self.lr_scheduler.milestones)))
           except:
